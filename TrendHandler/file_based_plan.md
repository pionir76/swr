...
SQLite 기반 트렌드 저장 방식과 병행하여 바이너리 파일 기반 레코딩을 구현합니다.  
...
| 6 | 6 | STIME | 녹화 시작 시각 (yymmddhhmmss, BCD) |
...
> 용량 사용률은 전용 API (`GET /api/tfile/usage`) 로 제공할 예정이며, 프론트는 100% 도달 시 UI에서 녹화 시작을 막습니다.
...
| 필드 | 타입 | 내용 |
|---|---|---|
| `usedBytes` | uint64 | 현재 녹화 파일 크기 (bytes) |
| `limitBytes` | uint64 | 최대 허용 크기 (4 GB) |
| `percent` | float | 사용률 (소수점 2자리, 예: 24.35) |

> **프론트 표시 목적**: 용량 사용률을 게이지/프로그레스바 형태의 그래프로 시각화.  
> 100% 도달 시 녹화 시작 버튼 비활성화 및 경고 표시.

### 10-3. `GET /api/tfile/files`
