...
PcLinkClient::PcLinkClient(const Model::DeviceConnection &connection, SerialBus &bus)
...
{
    if (!m_bus.isOpen()) {
        m_lastError = QStringLiteral("PCLink: serial bus not open");
        error = m_lastError;
        return false;
    }

    QByteArray response;
    if (!sendRequest(buildReadFrame(address, count), response, error))
        return false;
...
bool PcLinkClient::writeWords(int address, const QVector<quint16> &values, QString &error)
{
    ...
    QByteArray response;
    if (!sendRequest(buildWriteFrame(address, values), response, error))
        return false;
...
QByteArray PcLinkClient::buildWriteFrame(int address, const QVector<quint16> &values) const
{
    ...
    const QByteArray body =
        (stn + QLatin1String("WD") + QLatin1String("01") + addr + num + dataStr).toLatin1();
                                     ...
                                     QString &error) const
{
    ...
    if (response.size() < minLen) {
        const_cast<PcLinkClient *>(this)->m_lastError =
            QStringLiteral("PCLink read response too short");
        error = m_lastError;
        return false;
    }
    ...
    if (static_cast<quint8>(response.at(0)) != 0x02) {
        const_cast<PcLinkClient *>(this)->m_lastError =
            QStringLiteral("PCLink: missing STX in response");
        error = m_lastError;
        return false;
    }
    ...
    if (checksum(body) != receivedSum) {
        const_cast<PcLinkClient *>(this)->m_lastError =
            QStringLiteral("PCLink: checksum mismatch in read response");
        error = m_lastError;
        return false;
    }
    ...
    for (int i = 0; i < expectedCount; ++i) {
        ...
        if (!ok) {
            const_cast<PcLinkClient *>(this)->m_lastError =
                QStringLiteral("PCLink: invalid hex data in response");
            error = m_lastError;
            return false;
        }
        out.append(word);
...
bool PcLinkClient::parseWriteResponse(const QByteArray &response, QString &error) const
{
    if (response.isEmpty() || static_cast<quint8>(response.at(0)) != 0x06) {
        const_cast<PcLinkClient *>(this)->m_lastError =
            QStringLiteral("PCLink: write NAK or unexpected response");
        error = m_lastError;
        return false;
    }
    return true;
                               ...
                               QString &error)
{
    ...
    if (written != request.size() ||
        ...
        m_lastError = QStringLiteral("PCLink write failed: %1").arg(m_bus.errorString());
        error = m_lastError;
        return false;
    }
